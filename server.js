...
app.post('/submit-note', async (req, res) => {
  ...
  if (!itemId || !noteText || !noteText.trim()) {
    return res.status(400).json({ error: 'Item ID and valid note text are required' });
  }

  let formattedNote = `${noteText} [FIELD UPDATE]`;

  try {
    // Step 1: Create the update in Monday.com
    const updateQuery = `
      mutation {
        create_update(item_id: ${itemId}, body: "${formattedNote.replace(/"/g, '\\"')}") {
          id
        }
      }
    `;

    const updateResponse = await axios.post(apiUrl, { query: updateQuery }, {
      headers: {
        'Authorization': apiToken,
        'Content-Type': 'application/json',
        'API-Version': '2023-04'
    ...
    if (updateResponse.data.errors) {
      console.error('API errors in create_update:', updateResponse.data.errors);
      return res.status(500).json({ error: 'Failed to create update', details: updateResponse.data.errors });
    }

    console.log('Note submitted successfully:', updateResponse.data);
    const updateId = updateResponse.data.data.create_update.id;

    // Step 2: Notify specific users (Zach and Mark Young) using create_notification
    const userIdsToNotify = [45735202, 32273535]; // Zach and Mark Young's user IDs
    for (const userId of userIdsToNotify) {
      const notificationQuery = `
        mutation {
          create_notification(
            user_id: ${userId},
            target_id: ${updateId},
            text: "Update from field: \\"${noteText.replace(/"/g, '\\"')}\\"",
            target_type: Post
          ) {
            id
          }
      ...
      const notificationResponse = await axios.post(apiUrl, { query: notificationQuery }, {
        headers: {
          'Authorization': apiToken,
          'Content-Type': 'application/json',
          'API-Version': '2023-04'
      ...
      if (notificationResponse.data.errors) {
        console.error(`Failed to notify user ${userId}:`, notificationResponse.data.errors);
        return res.status(500).json({ error: `Failed to notify user ${userId}`, details: notificationResponse.data.errors });
      } else {
        console.log(`Notified user ${userId} successfully:`, notificationResponse.data);
      }
    }
